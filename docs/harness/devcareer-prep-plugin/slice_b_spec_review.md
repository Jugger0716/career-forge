...
- [ ] B-7. **`locked`의 기입 주체 규약이 없다(이 세션에서 관측된 신규 미결).** `origin`과
      `verification`은 이제 집행 코드가 있지만(아래 E-3 절), 생성 템플릿이 자기 노드에
      `locked: true`를 적으면 그 노드는 이후 재생성에서 **영원히 보존된다** — (g)가 닫으려는
      자기면제와 구조가 같다. **스펙이 `locked`의 기입 주체를 정한 적이 없어 이번 회차에 임의로
      제약을 넓히지 않고 기록만 남긴다**(조용히 넓히면 경계가 사라진다). `skills/career-from-git/`
      템플릿을 쓰는 회차가 결정 시한이다.
