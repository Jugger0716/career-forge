...
- [ ] E-3. `projectLedgerForSkills`의 **소비 지점**을 소스 스캔으로 관측한다 — 구현 7단계 (f)
      함수는 `store.mjs`에 만들었으나 호출자가 0곳이라 지금 단언을 넣으면 대상 0건인 공허한
      검사가 된다. `skills/career-from-git/`을 만들 때 「프롬프트 조립 지점이 원장 원본이 아니라
      이 함수를 거치는가」를 소스 스캔으로 관측하라. **이것을 잊으면 §6의 보조 방어가 선언만
      남는다** — M-1이 지적한 것과 같은 형태다.
