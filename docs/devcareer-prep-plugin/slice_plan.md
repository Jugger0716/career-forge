...
> **슬라이스 B가 신설한 스크립트 (2026-08-25 추가, 예외 표와 무관).** 위 표는 **슬라이스 A
> 파일을 고치는** 예외를 적는 자리이고, 슬라이스 B가 **새로 만드는** 파일은 예외가 아니다.
> 그래도 여기 적어 두는 이유는 위 `In scope` 열이 「… 외 11개」로 줄여 놓아 트리와 목록이
> 갈릴 때 다음 회차가 오독하기 때문이다.
>
> | 파일 | 무엇을 | 근거 |
> |---|---|---|
...
> **성능 처방 1(git diff 배치화)의 처리 회차 확정 (2026-09-01, 사용자 결정).**
> **슬라이스 C로 이연한다.** 콜드 리뷰 라운드 3(`perf_review.md` §2·§4)이 실측한 대로
> `scripts/lib/git.mjs`의 `getCommitFileChanges`는 **커밋 하나당 git 프로세스를 2개** 띄우고,
> 그 때문에 `--golden`이 300커밋 × 2런 = 약 1,200회 프로세스 기동에 시간을 거의 전부 쓴다
> (예상 70.1초 대 실측 65~70초로 구조가 시간을 설명한다). `git log --numstat --raw -z
> --diff-merges=first-parent` 한 쌍으로 배치화하면 **33.1초 → 1.35초(-95.9%)**이고,
> 300커밋 전량에 대해 경로·`changeType`·`oldPath`·`insertions`·`deletions`까지 **전값 일치
> 300/300**을 실측으로 확인했다.
>
> **그럼에도 이연하는 이유는 효과가 작아서가 아니라 절대 규칙 5다.** `scripts/lib/git.mjs`는
> 슬라이스 A가 만든 파일이고, 슬라이스 A 완료 이후 **수정 이력이 0건**이며, 위 예외 표 다섯 행
> 어디에도 없다. 규칙 5는 예외를 「그 항목이 회차 작업을 실제로 막을 때만」 허용하는데
> **성능 개선은 막지 않는다.** 여기서 예외를 열면 위 T4 14건 이연 결정과 같은 근거가 무너진다 —
> 「막지 않는데 겸사겸사 고치는 것은 이연 결정을 무효로 만든다」가 그 문단의 문면이다.
>
> **슬라이스 C 착수 시 이 문단이 그 작업 목록의 입력이다.** 반영 시 반드시 함께 밟을 것:
> ① `--diff-merges=first-parent`를 빠뜨리면 **머지 커밋의 `files[]`가 조용히 빈다**(`git log
> --raw`는 머지 diff를 기본으로 생략한다 — 실측 확인). ② 대조는 300커밋 픽스처 하나에서만
> 했으므로 바이너리·공백 경로·한글 경로·타입 변경(`T`) 시나리오로 다시 대조해야 한다.
> ③ 커밋 경계를 40자 hex로 식별하는 프로토타입 파서는 그대로 쓰지 마라 — 같은 형태의 파일명과
> 구별되지 않는다.

> **콜드 리뷰 라운드 2 잔여 처방 4건의 처리 회차 확정 (2026-09-02, 사용자 결정).**
> **넷 다 슬라이스 C로 이연한다.** 대상은 `cold_review_round2.md` §4 표의 처방
> **4 · 6 · 10 · 12**이며, 같은 문서 §4-1~§4-6이 반영한 8건(1·2·3·5·7·8·9·11)과 달리
> 이 넷은 착수하지 않는다.
>
> | # | 처방 | 이연 근거 | 비용 |
> |---|---|---|---|
> | 4 | `verify-evidence.mjs`의 `layerRefUnverifiable` fail-open을 닫는다 | **규칙 5** — 슬라이스 A 파일이고 예외 표 다섯 행 어디에도 없다 | S |
> | 6 | `PreToolUse` 훅으로 도구 호출 층을 막는다 | **규칙 5가 아니다 — 비용 L과 우선순위다**(아래 전제 확인 참조) | L |
> | 10 | `runLangCheck`의 fail-open을 닫는다 | **규칙 5** — 슬라이스 A 파일(`validate-plugin.mjs`)이고, 예외 2번은 `--secret-scan` 범위로 한정돼 이 건을 덮지 않는다 | S |
> | 12 | `--contamination`을 `npm test`에 잇는다 | **규칙 5** — `package.json`은 슬라이스 A이자 `PINNED_SLICE_A_FILES` 16건 중 하나라 핀까지 함께 고쳐야 한다 | S |
>
> **이연 근거는 「비싸서」가 아니라 일관성이다.** 넷 중 셋이 비용 S다. 그럼에도 예외를 열지
> 않는 이유는 이 표가 이미 두 번 같은 형태를 판정했기 때문이다 — T4 잔여 14건과 성능 처방
> 1(**-95.9%**)이다. 셋째 사례에서 반대로 가면 그 두 결정을 지탱한 문장(「막지 않는데
> 겸사겸사 고치는 것은 이연 결정을 무효로 만든다」)이 무너지고, **싸다는 것은 예외의 근거가
> 아니다.** 규칙 5는 「그 항목이 회차 작업을 실제로 막을 때만」을 요구하는데 넷 다 막지 않는다.
>
> **처방 4는 급박함이 실제로 줄었다.** 처방 7이 `write-artifact.mjs`(산출물이 디스크에 닿는
> 유일한 경로)에서 같은 축을 **무조건** 집행하므로, 쓰기와 렌더 두 지점이 이제 그 축을 본다.
> `verify-evidence`의 `hasFailures` 산식에 `layerRefUnverifiable`이 없는 fail-open 자체는
> 그대로 남으며, 그것이 이연이지 해소가 아닌 이유다.
>
> **처방 6의 전제를 확인했고, 결론이 표의 「미확정」을 뒤집는다.** `cold_review_round2.md`
> §4 표와 §6은 「`hooks/hooks.json`이 `plugin.json` 수정 없이 자동 발견되는지 확인할 수
> 없었다 — 자동 발견되지 않으면 슬라이스 A 예외 여부가 뒤집힌다」로 남겨 두었다. 설치된
> 플러그인 7종이 `hooks/hooks.json`을 갖고 있고, 확인한 5종(`superpowers`·`claude-security`·
> `hookify`·`ralph-loop`·`security-guidance`) **어느 것도 `plugin.json`에 `hooks` 필드를
> 선언하지 않는다.** 그중 `security-guidance`는 설명 자체가 편집·Stop 시점 훅이고 `hookify`는
> 훅 생성이 존재 이유이므로, 훅이 실제로 도는 플러그인들에서 그 관례가 성립한다.
> **따라서 처방 6은 `plugin.json`을 고칠 필요가 없고 슬라이스 A 예외도 필요 없다** —
> `hooks/hooks.json`은 신설 파일이며, 위 「슬라이스 B가 신설한 스크립트」 문단이 이미 같은
> 논리를 적용해 두었다. 그래도 이연하는 것은 비용 L과 우선순위 때문이고, **그 근거가 나머지
> 셋과 다르다는 사실을 여기 적는다.**
>
> **이 확인의 한계**: 설치본 **관례 관찰**이지 공식 문서 확인이 아니다(측정 세션에서 웹
> 접근을 쓸 수 없었다). 등급으로는 `source-scan` 수준의 간접 증거이고, 반증은 실제로
> `hooks/hooks.json`을 만들어 훅이 발화하는지 보는 것이다.
>
> **슬라이스 C 착수 시 이 문단이 그 작업 목록의 입력이다** — T4 잔여 14건·성능 처방 1과 같은
> 자격이다. 그리고 처방 10을 반영할 때는 `cold_review_round2.md` §4-7이 기록한 **두 번째
> fail-open 축**(`LANG_CHECK_SCHEMA_MISSING`이 `warning`이라 스키마 파일 삭제가 조용히
> 통과한다)을 같은 회차에 함께 닫아라 — 같은 함수이고 같은 예외 행을 쓴다.
