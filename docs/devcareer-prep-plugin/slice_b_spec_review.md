...
| **측정기 위생** | 4~7 | 부재를 예외가 아니라 FAIL로 (판독 지점 13곳) | 7번 직후 = **표적 검증 + 전면 콜드 리뷰 + 푸시 판단이 모이는 진짜 경계** |
...
7. **B류 11곳을 공유 헬퍼 1개로 한 커밋에 닫는다.** 규모 L.
   ...
   **왜 13번 앞인가**: 8단계가 스모크에 새 섹션을 추가하면서 깨진 판독 패턴을 복제하면 수리 대상이
   13곳이 아니라 20곳 이상이 된다.
8. **정본 이중화·미관측 분기 5건을 한 주제 한 커밋으로.** 규모 M.
...
전면 콜드 리뷰 대상은 `8f82d28..HEAD`(7번 완료 시점)이며, 그때 4~7번이 「관측 장치를 수리했다」는
하나의 일관된 단위가 되고 변이 관측도 전부 가능해져 리뷰어가 주장을 실행으로 확인할 수 있다.
