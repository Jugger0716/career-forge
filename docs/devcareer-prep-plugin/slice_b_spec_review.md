...
14. **구현 9단계 — 오염 스위트.** 규모 L. `tests/contamination/README.md`(사람↔스크립트 경계 정본)를
    **케이스보다 먼저** 쓴다 — 경계가 고정되기 전에 채점기를 만들면 자기가 만든 문자열을 자기가
    ...
    > - **`--contamination`·`--contamination-llm` 모드가 아직 배선되지 않았다.** 지금
    >   `main()`은 두 플래그를 알 수 없는 인자로 무시하고 기본 모드를 돌려 exit 0을 낸다.
    >   이 절이 관측하는 것은 **채점 로직과 케이스 집합의 형태**이고, 회차 산출물을 실제로
    >   읽어 채점하는 것은 그 모드의 몫이다. **미구현이지 미관측이 아니다.**
    > - **`runs/<run-id>/`가 아직 없다.** 기계 30건도 회차 산출물을 읽으므로(AC-8 (ii)),
    >   사람이 3회 실행하거나 오염 draft를 `write-artifact.mjs`로 통과시켜 재료화해야 한다.
    > - **`UC` 10건은 착수 전이다.** 사람 개입 산출물이 있어야 채점된다.
    > - **모드가 다섯 번째 게이트가 되려면 `slice_plan.md` 예외 표에 `package.json` 행이
    >   먼저 필요하다**(슬라이스 A). 지금은 그것이 회차 작업을 막지 않으므로 손대지 않았다.
