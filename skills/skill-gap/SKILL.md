...
1. **상위 계층이 먼저 있어야 한다 — 이제 쓰기 시점에 집행된다.** 지식맵은 career
   노드를 참조하고, 갭 리포트는 지식맵 노드를 참조한다. **상위 산출물이 저장 루트에
   없으면 `write-artifact.mjs`가 `PARENT_ARTIFACT_MISSING`으로 exit 2를 내고 아무것도
   쓰지 않는다**(라운드 2 처방 7). 실재하지 않는 상위 id를 인용하면
   `[LAYER_REF] LAYER_REF_UNRESOLVED` + exit 1이다. 둘 다 `--force`로 넘어갈 수 없다.

   사후 검사인 AC-14도 그대로 있다. 다만 **상위 산출물을 함께 넘기지 않으면 그쪽은
   위반이 아니라 `unverifiable`로 분류하고 그 배열은 종료 코드에 반영되지 않는다** —
   리포트상 위반 0건인데 사실은 미검증인 상태다. 그래서 8단계는 `--out-dir`로 전
   계층을 함께 넘긴다. 쓰기 시점 집행은 그 fail-open과 달리 **무조건**이다.
