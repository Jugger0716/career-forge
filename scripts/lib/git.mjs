...
/**
 * `git log <ref> [--since] [--until] --format=<LOG_FORMAT>` 결과를 파싱해
 * 커밋 메타데이터 배열을 반환한다. 파일 변경 집합(files[])은 포함하지
 * 않는다 — 그건 getCommitFileChanges()가 커밋별로 별도 호출해 채운다
 * (전체 로그를 한 번에 --numstat으로 받으면 머지 커밋에 대해 빈 출력이
 * 나오는 git log의 기본 동작과 부딪히므로, §2가 요구하는 "머지는 항상
 * 명시적 1부모 diff로 채운다" 규칙을 지키려면 커밋별 개별 diff 호출이
 * 필요하다 — 구현 5단계 산문 참조).
 *
 ...
 *
 * @param {string} repoPath
 * @param {{ref: "HEAD"|"--all", since?: string|null, until?: string|null}} opts
 ...
 */
export function listCommitMetadata(repoPath, { ref, since = null, until = null }) {
  const args = ["log", ref === "--all" ? "--all" : "HEAD"];
 ...
 *
 * @param {string[]} tokens
 * @returns {{changeType: "A"|"M"|"D"|"R", path: string, oldPath: string|null}[]}
 */
function parseNameStatusTokens(tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (code === "R" || code === "C") {
      ...
      throw new Error(
        `git.mjs parseNameStatusTokens: 알 수 없는 상태 코드입니다(status=${JSON.stringify(status)}) — ` +
        "-z 없이 파싱된 손상된 토큰 스트림이거나 지원하지 않는 name-status 코드입니다."
      );
    }
  }
  return out;
...
export function getCommitFileChanges(repoPath, sha, parents, isMerge) {
  ...
  if (numstatEntries.length !== nameStatusEntries.length) {
    throw new Error(
      `git.mjs getCommitFileChanges: numstat(${numstatEntries.length}건)과 name-status(${nameStatusEntries.length}건) ` +
      `파일 수가 다릅니다(sha=${sha}, base=${base}) — 두 호출이 같은 diff를 가리키지 않는 것으로 보입니다.`
    );
  }
