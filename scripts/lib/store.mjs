 ...
 * "경계 안에서의 오배치"가 아니다.
 *
 * **`.devcareer` 세그먼트가 심볼릭 링크(Windows 정션)여도 통과한다.** 이 함수는
 * `path.resolve`로 문자열만 정규화하고 `realpathSync`·`lstatSync`를 부르지
 * 않는다. 반면 이 검사를 통과한 경로에 실제로 쓰는 `writeJsonAtomic`·
 * `copyFileSync`는 링크를 따라가므로, 실경로로는 저장 루트 밖에 쓰일 수 있다.
 * **그럼에도 fs 조회를 넣지 않는다**: 링크를 심을 수 있는 주체는 이미 로컬
 * 파일시스템 쓰기 권한을 가졌고, 그 상대에게 문자열 검사는 애초에 방어선이
 * 아니다(바로 위 「이름만 `.devcareer`인 디렉터리」와 같은 수용 클래스다).
 * 이 함수가 막는 것은 적대적 주체가 아니라 **오케스트레이션이 조립한 잘못된
 * 문자열**이다. 다만 알려진 한계 목록이 실제 한계보다 좁으면 그것 자체가
 * 결함이므로 여기 적어 둔다.
 *
