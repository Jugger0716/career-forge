...
// facts.mjs`와 `scripts/lib/git.mjs`는 이 레포에 이미 구현돼 있지만, 이
// 파일은 그 둘을 전혀 참조하지 않는다 — 독립 재계산이 목적이므로 의도적으로
// 그렇게 만들었다. 즉 이 스크립트가 만드는 골든은 "잘못 구현된 수집기의
// 출력 스냅샷"이 아니라 리터럴로부터의 독립 재계산이다.
//
// 사용법:
...
// ---------------------------------------------------------------------------
// git 플럼빙 — §7 고정 프리픽스. scripts/lib/git.mjs는 이미 구현돼 있지만
// 이 파일은 그것을 재사용하지 않는다 — "수집기·검증기와 독립적으로
