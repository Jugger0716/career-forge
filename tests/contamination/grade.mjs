...
//      바꾸기」 같은 변이를 관측하는 데 픽스처 빌드가 통째로 걸린다 — 전 시나리오
//      콜드 빌드 **109초** 실측(`docs/devcareer-prep-plugin/perf_review.md` §3).
//      **초판 주석은 이것을 「10분」이라 적었고 그것은 측정된 값이 아니었다** —
//      논거는 그대로 서지만 배율이 5배 넘게 과장돼 있었다.
