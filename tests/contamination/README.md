...
#### 자식 계층 케이스는 이제 상위 계층을 먼저 요구한다 (2026-09-02, 라운드 2 처방 7)

**선행 조건이 하나 늘었다.** `write-artifact.mjs`가 쓰기 시점에 `parentRefs`를 해소하므로,
`knowledge-map`·`gap-report` 케이스를 재료화하려면 **그 케이스 루트에 상위 계층이 먼저
기록돼 있어야 한다.** 없으면 `PARENT_ARTIFACT_MISSING` + exit 2이고 `--force`로도 넘어갈 수
없다. 절대 규칙 1이 재료화도 이 CLI를 거치게 못 박으므로 우회로는 없다.

대상은 5건이다 — `SB-06`·`SB-07`·`SB-09`가 `gap-report`, `SB-08`·`SB-10`이 `knowledge-map`.

**현재 추적된 회차 잔여물은 이 절차 이전에 만들어졌다 — 감추지 않는다.** `runs/<runId>/secret/<caseId>/`
아래 루트에는 계층 파일 하나와 `state.json`만 있고, 그 `parentRefs`는 어디에도 실재하지 않는
`ct-sb-parent`를 가리킨다(실측: 5건 × 3회차 = 15개 파일). **채점 경로는 그 파일들을 읽기만
하므로 `--contamination`은 오늘 빨개지지 않는다.** 즉 지금 갈려 있는 것은 게이트가 아니라
**절차서와 잔여물**이고, 이 문단이 그 사실의 기록이다.

**재료화를 다시 하는 회차는 아직 정해지지 않았다(사용자 결정, 2026-09-02).** 이번 회차는
선행 조건만 적고 재수행을 미뤘다 — 추적되는 게이트 재료 15개 파일을 바꾸는 일이고 §1이
재료화를 사람의 몫으로 못 박았기 때문이다. 다시 할 때는 케이스 루트에 상위 계층 체인을
먼저 기록하고, `parentRefs`가 그 실재하는 id를 가리키도록 케이스 파일도 함께 고쳐야 한다.
...
> **자식 계층 케이스의 선행 조건(2026-09-02).** `SB-06`~`SB-10` 중 5건은 상위 계층이 케이스
> 루트에 먼저 기록돼 있어야 재료화된다(§2-3 참조). 현재 추적된 잔여물은 그 절차 이전 것이며
> 재수행 회차는 미정이다.
